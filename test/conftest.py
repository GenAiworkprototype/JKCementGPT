...
os.environ.setdefault("PYTHONPATH", str(pathlib.Path(__file__).resolve().parents[1] / "cementGPT_llm_chat"))
...
def stub_model_loader(monkeypatch):
    # Patch both module paths to cover imports via `utils.model_loader` and `cementGPT_llm_chat.utils.model_loader`
    import cementGPT_llm_chat.utils.model_loader as ml_mod
    from cementGPT_llm_chat.utils import model_loader as ml_mod2

    class FakeApiKeyMgr:
        def __init__(self):
            self.api_keys = {"GOOGLE_API_KEY": "y"}
    ...
    import cementGPT_llm_chat.src.document_ingestion.data_ingestion as di
    import cementGPT_llm_chat.src.document_chat.retrieval as r
